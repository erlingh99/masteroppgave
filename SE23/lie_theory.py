...
class SO2xR2(LieGroup):
    ...
    def Exp(cls, tau):
        ...
        rho_vec = tau[1:]
        theta = tau[0]
        return SO2xR2(SO2.Exp(theta), rho_vec)
    ...
    def copy(self):
        return SO2xR2(self.R.copy(), self.t.copy())
...
class SO3xR3xR3(LieGroup):
    ...
    def copy(self):
        return SO3xR3xR3(self.R.copy(), self.v.copy(), self.p.copy())
    ...
    @classmethod
    def from_matrix(cls, T):
        return cls(SO3.from_matrix(T[:3, :3]), T[:3, 3], T[:3, 4])

    @classmethod
    def Exp(cls, tau) -> "LieGroup":
        return cls(SO3.Exp(tau[:3]), tau[3:6], tau[6:])
